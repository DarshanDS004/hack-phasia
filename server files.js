...
// Function to analyze medical report text with OpenAI
async function analyzeReportWithAI(reportText) {
    console.log('🤖 Starting AI analysis...');
    console.log('📝 Text length:', reportText.length, 'characters');

    // Limit input text to prevent high costs (max ~3000 chars)
...
app.get('/api/test', (req, res) => {
    ...
    res.json({ 
        message: 'Backend server running with AI integration!',
        database: db ? 'Connected' : 'Not connected (using localStorage)',
        openai: openaiConfigured ? 'API key configured' : 'API key missing',
...
app.listen(PORT, () => {
    ...
    console.log('Ready for AI-powered medical report analysis!');
});